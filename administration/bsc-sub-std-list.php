...
<main id="main" class="main">
    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">BSC Admitted Students (Subject Wise)</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section">
        ...
        <div class="col-lg-6 mx-auto">
            ...
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">BSC Admitted Students (Subject Wise)</h5>
                    ...
                    <form class="row g-3 needs-validation" method="post" action="all-applicants-list.php" target="_blank">
                        ...
                        <div class="input-group col-md-12">
                            ...
                            <select class="form-select" name="cr_name" aria-label="Default select example" required>
                                ...
                                <?php
                                $id = $_GET['id'];
                                $select = "select * from program_subject,subjects,program where program_subject.prgm_id = program.prgm_id AND subjects.sub_id = program_subject.sub_id AND program_subject.prgm_id = $id";
                                $stmt = $con->prepare($select);
                                $stmt->execute();

                                while ($fetch_merit = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                ?>
                                    <option value="<?= $fetch_merit['sub_id'] ?>"><?= $fetch_merit['sub_name'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
